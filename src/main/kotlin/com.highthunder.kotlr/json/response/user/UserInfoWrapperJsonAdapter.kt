 ...
 * @version 1.0.0
 */
class UserInfoWrapperJsonAdapter(moshi: Moshi) {
    ...
    fun fromJson(reader: JsonReader): ResponseUserInfo.Wrapper {
        return when (reader.peek()) {
            JsonReader.Token.BEGIN_OBJECT -> ResponseUserInfo.Wrapper(response = responseAdapter.fromJson(reader))
            ...
            else -> throw JsonDataException("Expected a field of type Object, List or String but got ${reader.peek()}")
        }
    }
     ...
     */
    @ToJson
    fun toJson(writer: JsonWriter, value: ResponseUserInfo.Wrapper?) {
        if (value?.error != null) {
